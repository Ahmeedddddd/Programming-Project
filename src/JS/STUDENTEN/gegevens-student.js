...
class StudentGegevens {
    ...
    async loadStudentGegevens() {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                const errorResult = await response.json().catch(() => ({ message: response.statusText || 'Onbekende fout' }));
                console.error('❌ Fout bij laden studentgegevens, respons:', errorResult);
    ...
    async updateStudentGegevens(formData) {
        ...
        try {
            ...
            const result = await response.json().catch(() => {
                ...
                return { success: false, message: response.statusText || 'Server respons was geen geldige JSON of leeg.' };
            });

            console.log('📝 Update result:', result);

            if (response.ok && result.success) { // Combineer checks voor robuustheid
                this.studentData = result.data;
                this.displayStudentGegevens();
                this.displayProjectInfo();
                ...
                console.error('❌ Fout bij bijwerken, HTTP Status:', response.status, 'Status Text:', response.statusText, 'Result:', result);
                throw new Error(result.message || `Update mislukt (HTTP ${response.status})`);
            }
        ...
        if (githubLink) {
            ...
            if (!githubLink.querySelector('input')) {
                if (data.githubUrl) {
                    ...
                    githubLink.innerHTML = '<strong>GitHub:</strong> <span class="no-link">Geen GitHub link</span>';
                }
            }
        }
        ...
        if (cvLink) {
            ...
            if (!cvLink.querySelector('input')) {
                if (data.cvUrl) {
                    ...
                    cvLink.innerHTML = '<strong>CV:</strong> <span class="no-link">Geen CV geüpload</span>';
                }
            }
        }
        ...
        if (linkedinLink) {
            ...
            if (!linkedinLink.querySelector('input')) {
                if (data.linkedinUrl) {
                    ...
                    linkedinLink.innerHTML = '<strong>LinkedIn:</strong> <span class="no-link">Geen LinkedIn link</span>';
                }
            }
        }
    }
    ...
    updateProjectStats(data) {
        ...
        if (projectStats) {
            ...
            tafelNrElement.innerHTML = `<strong>Tafel:</strong> <span class="field-value">${data.tafelNr || 'Niet toegewezen'}</span>`;
            ...
            leerjaarElement.innerHTML = `<strong>Leerjaar:</strong> <span class="field-value">${data.leerjaar || 'Niet ingevuld'}</span>`;
            ...
            opleidingElement.innerHTML = `<strong>Opleiding:</strong> <span class="field-value">${data.opleiding || 'Niet ingevuld'}</span>`;
        ...
        projectFields.forEach(fieldId => {
            ...
            if (field && this.studentData) {
                ...
                if (fieldId === 'project-beschrijving' || fieldId === 'over-mezelf') {
                    ...
                    const inputType = fieldId.includes('url') ? 'url' : (fieldId === 'tafel-nummer' ? 'number' : 'text');
    ...
    createEditableFields() {
        ...
        editableFields.forEach(field => {
            ...
            const projectRelatedFields = ['project-titel', 'project-beschrijving', 'over-mezelf', 'github-url', 'cv-url', 'linkedin-url', 'tafel-nummer'];
    ...
    async handleFormSubmit() {
        ...
        const formData = {};
        const editInputs = document.querySelectorAll('.editable-field:not([data-field^="project-"]):not([data-field="github-url"]):not([data-field="cv-url"]):not([data-field="linkedin-url"]):not([data-field="tafel-nummer"]) .edit-input');
    ...
    showSuccess(message) {
        ...
        if (window.showNotification) {
            ...
            // Fallback for environments without notification system
            console.log('Notification system not available, showing alert: ' + message);
        }
    }
