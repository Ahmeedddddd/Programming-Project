console.log('🔎 ZoekbalkHomepage voor studenten loaded');

// Global caches (alleen bedrijven en projecten)
window.entityCaches = {
  bedrijven: { data: [], timestamp: 0, maxAge: 5 * 60 * 1000 },
  projecten: { data: [], timestamp: 0, maxAge: 5 * 60 * 1000 }
};

class ZoekbalkStudentHomepage {
  constructor() {
  ...
  async loadData(key, endpoint) {
    ...
    try {
      ...
      if (key === 'bedrijven') {
        cache.data = [
          { bedrijfsnummer: 'static-1', naam: 'Voorbeeld Bedrijf', sector: 'IT', gemeente: 'Brussel' }
        ];
      } else if (key === 'projecten') {
        cache.data = [
          { projectId: 'static-1', projectTitel: 'Voorbeeld Project', projectBeschrijving: 'Beschrijving...', studentNaam: 'Naam Student' }
        ];
      } else {
        cache.data = [];
      }
  ...
  updateDatalist(term) {
    ...
    const { bedrijven, projecten } = window.entityCaches;
    const allNames = [
    ...
    const opts = [...new Set(
      allNames.filter(n => n && n.toLowerCase().startsWith(term)).slice(0, 10)
    )];
  ...
  renderResults(items, term) {
    ...
    items.forEach(item => {
      ...
      const title = item._type === 'bedrijf'
        ? item.naam
        : item.projectTitel;
...
// start alleen op de studenten homepage!
window.addEventListener('DOMContentLoaded', () => new ZoekbalkStudentHomepage());
